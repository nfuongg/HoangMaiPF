    ...
    <main className="max-w-[1920px] w-full mx-auto relative bg-background overflow-clip font-heading text-primary">
      ...
      <nav className="sticky top-6 z-50 mx-4 md:mx-[clamp(16px,2vw,38px)] flex flex-row justify-between items-center p-[15px] rounded-[16px] shadow-[inset_0_0_0_1px_#0D2B45] bg-background/80 backdrop-blur-md">
        <div className="flex flex-row items-center gap-[18px]">
          <p className="text-[clamp(20px,2.4vw,46px)] font-extrabold leading-none text-primary">
            PORTFOLIO
          </p>
      ...
      >
        <div className="rounded-[20px] shadow-[inset_0_0_0_1px_#0D2B45] p-8 md:p-12 bg-muted">
          <p className="text-secondary font-bold text-[13px] uppercase tracking-widest mb-3">
            🎯 Mục tiêu bài học
          </p>
          <p className="text-[clamp(18px,2vw,32px)] font-light leading-[1.7] text-primary">
            {assignment.goal}
          </p>
        </div>
      </motion.section>
      ...
      <section className="mt-[clamp(30px,5vw,80px)] mx-4 md:mx-10 flex flex-col gap-[clamp(24px,4vw,60px)] relative z-10 pb-[clamp(60px,10vw,150px)]">
        {assignment.sections.map((section, si) => (
          ...
          >
            ...
            {/* Content */}
            {section.content && (
              <div className="rounded-[20px] shadow-[inset_0_0_0_1px_#0D2B45] p-8 bg-muted">
                <p className="text-[clamp(15px,1.3vw,22px)] font-light leading-[1.8] text-primary">
                  {section.content}
                </p>
              </div>
            )}

            {/* Table - đổi màu sang tông xanh */}
            {section.table && (
                ...
                <table className="min-w-full text-sm border-collapse">
                  ...
                  <tbody>
                    {section.table.rows.map((row, rIdx) => (
                      <tr
                        key={rIdx}
                        className={rIdx % 2 === 0 ? "bg-muted/50" : "bg-background"}
                      >
                        {row.map((cell, cIdx) => (
                          <td
                            key={cIdx}
            ...
            {section.steps && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {section.steps.map((s, i) => (
                  <div
                    key={i}
                    className="flex flex-row items-start gap-4 p-6 rounded-[16px] shadow-[inset_0_0_0_1px_#0D2B45] bg-muted hover:bg-muted/80 transition-all"
                  >
                    ...
                    <div className="flex flex-col gap-1">
                      <p className="font-bold text-[clamp(15px,1.2vw,20px)] text-primary">
                        {s.title}
                      </p>
                      <p className="text-[clamp(13px,1vw,16px)] font-light leading-[1.6] text-primary/70">
                        {s.desc}
                      </p>
                    </div>
                  </div>
            ...
            {section.highlights && (
              <div className="flex flex-col gap-3">
                {section.highlights.map((h, i) => (
                  <div
                    key={i}
                    className="flex flex-row items-start gap-4 p-5 rounded-[14px] bg-muted shadow-[inset_0_0_0_1px_#0D2B45]"
                  >
                    <span className="text-2xl shrink-0">{h.emoji}</span>
                    <p className="text-[clamp(14px,1.1vw,18px)] font-light leading-[1.6] text-primary">
        ...
        >
          <Link
            to="/#project"
            className="flex flex-row items-center gap-2 py-3 px-6 rounded-[57px] shadow-[inset_0_0_0_1px_#0D2B45] hover:bg-muted transition-all font-bold text-[clamp(14px,1.2vw,18px)] text-primary"
          >
